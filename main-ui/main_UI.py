...
class Fatigue_detecting(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.yawn_checkBox1 = wx.CheckBox(sbSizer3.GetStaticBox(), wx.ID_ANY, u"嘴部疲劳检测", wx.Point(-1, -1),
                                          wx.Size(-1, 15), 0)
        self.yawn_checkBox1.SetValue(True)
        bSizer5.Add(self.yawn_checkBox1, 0, wx.ALL, 5)
        self.blink_checkBox2 = wx.CheckBox(sbSizer3.GetStaticBox(), wx.ID_ANY, u"眼部疲劳检测", wx.Point(-1, -1),
                                           wx.Size(-1, 15), 0)
        self.blink_checkBox2.SetValue(True)
        bSizer5.Add(self.blink_checkBox2, 0, wx.ALL, 5)
        sbSizer3.Add(bSizer5, 1, wx.EXPAND, 5)
        bSizer6 = wx.BoxSizer(wx.HORIZONTAL)
        self.nod_checkBox7 = wx.CheckBox(sbSizer3.GetStaticBox(), wx.ID_ANY, u"点头瞌睡疲劳检测", wx.Point(-1, -1),
                                         wx.Size(-1, 15), 0)
        ...
        sbSizer4 = wx.StaticBoxSizer(wx.StaticBox(sbSizer1.GetStaticBox(), wx.ID_ANY, u"注意力检测"), wx.VERTICAL)
        bSizer8 = wx.BoxSizer(wx.HORIZONTAL)
        self.m_checkBox4 = wx.CheckBox(sbSizer4.GetStaticBox(), wx.ID_ANY, u"注意力检测", wx.DefaultPosition,
                                       wx.Size(-1, 15), 0)
        self.m_checkBox4.SetValue(True)
        bSizer8.Add(self.m_checkBox4, 0, wx.ALL, 5)
        self.m_staticText2 = wx.StaticText(sbSizer4.GetStaticBox(), wx.ID_ANY, u"注意力检测(秒):", wx.DefaultPosition,
                                           wx.Size(-1, 15), 0)
        ...
        self.m_listBox21.Bind(wx.EVT_LISTBOX, self.OUT_AR_CONSEC_FRAMES)  # 注意力时间设置
        ...
        while self.cap.isOpened():
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    打哈欠
                    """
                    if self.yawn_checkBox1.GetValue():
                        # 嘴巴坐标
                    ...
                    眨眼
                    """
                    if self.blink_checkBox2.GetValue():
                        # 提取左眼和右眼坐标
                    ...
                    if self.nod_checkBox7.GetValue():
                        ...
                        for start, end in self.line_pairs:
                            cv2.line(img=im_rd, pt1=(int(reprojectdst[start][0]), int(reprojectdst[start][1])),
                                     pt2=(int(reprojectdst[end][0]), int(reprojectdst[end][1])),
                                     color=(0, 0, 255), thickness=1)
                        print(f'pitch: {euler_angle[0, 0]}, yaw: {euler_angle[1, 0]}, roll: {euler_angle[2, 0]}')
                ...
                if self.oCOUNTER >= self.OUT_AR_CONSEC_FRAMES_check:
                    self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"注意力不集中!!!\n")
                    self.oCOUNTER = 0
            ...
            if self.TOTAL >= 50 or self.mTOTAL >= 15 or self.hTOTAL >= 30:
                ...
                if not self.check_send_waring:
                    self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u'进行语音通报检测者进入疲劳状态！！！\n')
                    self.warning()
                    self.send_info()
                    self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u'疲劳检测结果已发送至您的短信！！！\n')
                    self.check_send_waring = True
    ...
    def cameraid_choice(self, event):
        ...
        if cameraid == 0:
            ...
            self.m_textCtrl3.AppendText(u"准备打开手机IP摄像头!!!\n")
            self.VIDEO_STREAM = 'http://admin:981222@192.168.10.16:8081/'

    def vedio_on(self, event):
        if self.CAMERA_STYLE:  # 释放摄像头资源
            # 弹出关闭摄像头提示窗口
    ...
    def OUT_AR_CONSEC_FRAMES(self, event):
        self.m_textCtrl3.AppendText(u"设置注意力检测间隔为:\t" + event.GetString() + "秒\n")
...
class main_app(wx.App):
    """
     在 OnInit() 里边申请Frame类，这样能保证一定是在app后调用，
